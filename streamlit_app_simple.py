...
def main():
    ...
    st.title("🌟 Celebrity Look-Alike Challenge")
    st.markdown("Take a photo with your camera to find your celebrity match!")
    ...
    try:
        ...
        if len(celeb_data) == 0:
            st.warning("⚠️ No celebrities found in the database. Please add celebrity images to the 'celebs/' directory.")
    ...
    with col2:
        ...
        if camera_image and 'match' in locals() and match:
        ...
        with st.form("register_form"):
            name_input = st.text_input("Enter name:", placeholder="John Doe")
            ...
            if submitted and camera_image:
                try:
                    ...
                    if face_encs and img_bgr is not None:
                        ...
                        st.warning("⚠️ No face detected. Please take a new photo with a clear face.")
                except Exception as e:
    ...
    st.sidebar.metric("Total Celebrities", len(celeb_data))
    
    if camera_image and 'match' in locals() and match:
        st.sidebar.metric("Current Match", match['name'])
        st.sidebar.metric("Similarity Score", f"{similarity:.1f}%")
    
    st.sidebar.markdown("---")
    st.sidebar.markdown("### ℹ️ Instructions")
    st.sidebar.markdown("""
    1. **Take Photo**: Click the camera button to capture your photo
    2. **View Match**: See your celebrity match instantly
    3. **Register**: Use the form to add new faces to the database
    """)
